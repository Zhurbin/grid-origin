...
const useSelection = ({ gridRef, initialActiveCell = null, initialSelections = EMPTY_SELECTION, columnCount = 0, rowCount = 0, selectionPolicy = "multiple", newSelectionMode = "clear", allowDeselectSelection = true, onFill, isHiddenRow = defaultIsHidden, isHiddenColumn = defaultIsHidden, alwaysScrollToActiveCell = true, selectionTopBound = 0, selectionBottomBound = rowCount - 1, selectionLeftBound = 0, selectionRightBound = columnCount - 1, mouseDownInterceptor, mouseMoveInterceptor, mergedCells = [], canSelectionSpanMergedCells = defaultSelectionSpan, getValue, onSelectionMove, }) => {
    ...
    const isSelecting = react_1.useRef(false);
    const isFilling = react_1.useRef(false);
    ...
    const draggedSelectionIndex = react_1.useRef();
    /**
    ...
    const activeSelectionsRef = react_1.useRef(selections);
    const mergedCellsRef = react_1.useRef(mergedCells);
    react_1.useEffect(() => {
    ...
    const isCellOutOfBounds = react_1.useCallback((cell) => {
    ...
    const cellIndexInSelection = (cell, selections) => {
        const id = helpers_1.cellIdentifier(Math.max(selectionTopBound, cell.rowIndex), Math.max(selectionLeftBound, cell.columnIndex));
    ...
    const handleMouseDown = react_1.useCallback((e) => {
        ...
        const isSameAsActiveCell = helpers_1.isEqualCells(coords, activeCell);
    ...
    const handleMouseMove = react_1.useCallback((e) => {
        ...
        if (helpers_1.isEqualCells(firstActiveCell.current, coords)) {
    ...
    const keyNavigate = react_1.useCallback((direction, modify, metaKeyPressed) => {
        ...
        switch (direction) {
            case types_1.Direction.Up:
                ...
                rowIndex = helpers_1.clampIndex(Math.max(rowIndex - 1, selectionTopBound), isHiddenRow, direction);
                // Shift + Ctrl/Commmand
                ...
                if (metaKeyPressed) {
                    ...
                    rowIndex = helpers_1.findNextCellInDataRegion(startCell, getValue, isHiddenRow, direction, selectionTopBound);
                }
                break;
            case types_1.Direction.Down:
                if (isMergedCell)
                    rowIndex = currentBounds.bottom;
                rowIndex = helpers_1.clampIndex(Math.min(rowIndex + 1, selectionBottomBound), isHiddenRow, direction);
                // Shift + Ctrl/Commmand
                if (metaKeyPressed) {
                    const startCell = {
                        ...currentCell,
                    ...
                    rowIndex = helpers_1.findNextCellInDataRegion(startCell, getValue, isHiddenRow, direction, selectionBottomBound);
                }
                break;
            case types_1.Direction.Left:
                if (isMergedCell)
                    columnIndex = currentBounds.left;
                columnIndex = helpers_1.clampIndex(Math.max(columnIndex - 1, selectionLeftBound), isHiddenColumn, direction);
                // Shift + Ctrl/Commmand
                if (metaKeyPressed) {
                    const startCell = {
                        ...currentCell,
                    ...
                    columnIndex = helpers_1.findNextCellInDataRegion(startCell, getValue, isHiddenColumn, direction, selectionLeftBound);
                }
                break;
            case types_1.Direction.Right:
                if (isMergedCell)
                    columnIndex = currentBounds.right;
                columnIndex = helpers_1.clampIndex(Math.min(columnIndex + 1, selectionRightBound), isHiddenColumn, direction);
                // Shift + Ctrl/Commmand
                if (metaKeyPressed) {
                    const startCell = {
                        ...currentCell,
                    ...
                    columnIndex = helpers_1.findNextCellInDataRegion(startCell, getValue, isHiddenColumn, direction, selectionRightBound);
                }
                break;
        }
        ...
        const isUserNavigatingToActiveCell = helpers_1.isEqualCells(firstActiveCell.current, coords);
    ...
    // Ctrl+Space
    const selectColumn = react_1.useCallback(() => {
    ...
    // Shift+Space
    const selectRow = react_1.useCallback(() => {
        if (!selectionEnd.current || !selectionStart.current)
            return;
        selectionStart.current = {
            rowIndex: selectionStart.current.rowIndex,
    ...
    const handleFillHandleMouseUp = react_1.useCallback((e) => {
        ...
        if (!newBounds)
            return;
        /* Focus on the grid */
        (_b = gridRef.current) === null || _b === void 0 ? void 0 : _b.focus();
        /* Callback */
    ...
    const fillHandleProps = react_1.useMemo(() => {
        return {
     ...
     * Ond drag move
     */
    const handleSelectionMouseMove = react_1.useCallback((e) => {
        var _a;
        if (!(gridRef === null || gridRef === void 0 ? void 0 : gridRef.current))
            return;
        const coords = gridRef.current.getCellCoordsFromOffset(e.clientX, e.clientY);
     ...
     */
    const handleSelectionMouseUp = react_1.useCallback(() => {
        var _a;
        ...
        if (sel) {
            ...
            selectionStart.current = coords;
            if (helpers_1.selectionSpansCells(sel === null || sel === void 0 ? void 0 : sel.bounds)) {
                setSelections((prev) => {
